...
spark = SparkSession.builder.appName("MW part-one/firstSection minio 30.txt").config('spark.driver.host','spark-edge-vm0.service.consul').config(conf=conf).getOrCreate()
...
splitDF.write.mode("overwrite").csv("s3a://mwaghela/30-csv")
splitDF.write.mode("overwrite").format("json").save("s3a://mwaghela/30-json")
splitDF.write.mode("overwrite").format("csv").option("compression", "lz4").save("s3a://mwaghela/30-csv-lz4")
