...
spark = SparkSession.builder.appName("MW part-one/SectionSection minio-read-60.txt").config('spark.driver.host','spark-edge-vm0.service.consul').config(conf=conf).getOrCreate()
...
splitDF.write.mode("overwrite").csv("s3a://mwaghela/60-csv")

splitDF.write.mode("overwrite").parquet("s3a://mwaghela/60-parquet")
