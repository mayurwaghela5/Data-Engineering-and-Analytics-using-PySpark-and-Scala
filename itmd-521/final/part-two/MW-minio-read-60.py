...
JsonSchema = splitDF.coalesce(1).write.format("json").mode("overwrite").save("s3a://mwaghela/60-csvTOjson")
jsondf = spark.read.json("s3a://mwaghela/60-csvTOjson")
#json printschema
jsondf.printSchema()
jsondf.show(10)

ParquetSchema = splitDF.coalesce(1).write.format("parquet").mode("overwrite").save("s3a://mwaghela/60-csvTOparquet")
parquetdf = spark.read.parquet("s3a://mwaghela/60-csvTOparquet")
#parquet printschema
