...
spark = SparkSession.builder.appName("MW part-two/minio-read-80").config('spark.driver.host','spark-edge-vm0.service.consul').config(conf=conf).getOrCreate()

# Read the csv datatype into a DataFrame
csvdf = spark.read.csv('s3a://mwaghela/80-csv').cache()
...
JsonSchema = splitDF.coalesce(1).write.format("json").mode("overwrite").save("s3a://mwaghela/80-csvTOjson")
jsondf = spark.read.json("s3a://mwaghela/80-csvTOjson")
#json printschema
jsondf.printSchema()
jsondf.show(10)

ParquetSchema = splitDF.coalesce(1).write.format("parquet").mode("overwrite").save("s3a://mwaghela/80-csvTOparquet")
parquetdf = spark.read.parquet("s3a://mwaghela/80-csvTOparquet")
#parquet printschema
