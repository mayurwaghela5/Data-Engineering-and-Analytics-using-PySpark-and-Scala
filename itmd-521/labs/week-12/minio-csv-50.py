...
spark = SparkSession.builder.appName("Mayur 50 test").config('spark.driver.host','spark-edge-vm0.service.consul').config(conf=conf).getOrCreate()
...
splitDF.write.mode("overwrite").parquet("s3a://mwaghela/50-parquet")
splitDF.write.mode("overwrite").csv("s3a://mwaghela/50-csv")
