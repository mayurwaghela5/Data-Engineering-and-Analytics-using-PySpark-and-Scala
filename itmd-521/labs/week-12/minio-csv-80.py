...
spark = SparkSession.builder.appName("Mayur 80 test").config('spark.driver.host','spark-edge-vm0.service.consul').config(conf=conf).getOrCreate()
...
splitDF.write.mode("overwrite").parquet("s3a://mwaghela/80-parquet")
splitDF.write.mode("overwrite").csv("s3a://mwaghela/80-csv")
